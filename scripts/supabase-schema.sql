...
alter table public.photographers add column if not exists username text unique;
alter table public.photographers add column if not exists "isPublic" boolean not null default true;
...
do $$
begin
  alter table public.photographers drop constraint if exists photographers_username_format_check;
  alter table public.photographers add constraint photographers_username_format_check
    check (username is null or username ~ '^[a-z0-9-]{2,80}$');
exception
  when undefined_table then
    null;
end $$;
