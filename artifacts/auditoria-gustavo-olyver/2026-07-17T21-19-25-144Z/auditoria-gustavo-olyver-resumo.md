...
Gerado em: 2026-07-17T17:19:25 (America/Porto_Velho)  
Banco consultado até: 2026-07-17T17:19:20 (America/Porto_Velho)  
...
| Período analisado | 2026-05-26T18:46:00 até 2026-07-17T17:19:20 |
| Primeira venda localizada no banco | 2026-05-26T18:46:00 |
| Venda mais recente localizada no banco | 2026-07-15T19:39:13 |
| Primeira / última venda no total definitivo | 2026-05-29T22:50:39 / 2026-07-15T19:39:13 |
| Pedidos pagos definitivos | 17 |
| Fotos/itens vendidos definitivos | 30 |
| Subtotal original alocado (estimado) | R$ 447,00 |
| Valor bruto cobrado nos itens | R$ 384,35 |
| Descontos alocados (estimados) | R$ 62,65 |
| Valor efetivamente recebido atribuído aos itens | R$ 384,35 |
| Taxas financeiras do gateway | Não persistidas de forma normalizada |
| Comissão FunPace comprovada | R$ 153,72 |
| Valor total devido ao fotógrafo antes de repasses | R$ 230,63 |
| Saldo interno adicional em quarentena (gateway pending) | R$ 136,89 |
| Saldo líquido total registrado internamente | R$ 367,52 |
| Valor já repassado com evidência mínima | R$ 0,00 |
| Saldo final pendente comprovável | **R$ 230,63** |
...
Itens conciliados/avaliados: 80; itens válidos segundo os registros internos: 50; itens incluídos no total definitivo após a checagem atual do gateway: 30; itens excluídos ou em quarentena: 50.

### Conciliação atual com a InfinitePay

Foram tentados 36 pedidos pagos: 6 confirmados como paid pelo endpoint `payment_check`, 11 sem identificadores suficientes, 0 falhas de consulta e 19 respostas não pagas. Os 19 pedidos que retornaram pending representam R$ 228,13 brutos / R$ 136,89 líquidos e foram retirados do total definitivo, embora continuem registrados como pagos no banco. A consulta foi somente de verificação e não alterou pedidos ou pagamentos. Detalhes: `auditoria-gustavo-olyver-infinitepay.csv`.

## E. Regra de comissão

O código atual calcula por item `platformFee = price × platformFeePercent` e `netAmount = price - platformFee`. A implementação efetiva usa o percentual global de `platform_settings` no fulfillment; o documento arquitetural menciona override por `photographers.commissionPercent`, mas esse override não é aplicado pela implementação atual. Configuração atual: 40.00%; comissão específica do Gustavo: nula. Fontes/rates observadas nos itens definitivos: 40.00|photographer_transactions, 39.97|photographer_transactions (variações decorrem do arredondamento por item). Uma cópia de 2026-06-01 também registra 40%, corroborando essa regra desde aquela data; não existe histórico temporal estruturado para provar períodos anteriores. Por isso os valores persistidos por item/transação prevalecem.

## F. Total por evento

Arquivo: `auditoria-gustavo-olyver-eventos.csv` (9 linhas).
...
Excluídos: 50. Divergências/observações: 50. Consulte `auditoria-gustavo-olyver-excluidos.csv` e `auditoria-gustavo-olyver-divergencias.csv`.
...
| Cálculo | Valor |
|---|---:|
| Total bruto cobrado nas vendas do Gustavo | R$ 384,35 |
| (-) Comissão da FunPace | R$ 153,72 |
...
Saldo adicional em quarentena: R$ 136,89. Se os 19 pedidos atualmente pending forem confirmados no portal/extrato, o saldo interno total passa a R$ 367,52.
...
**Médio.** A confiança considera a presença de pedido pago, evidência adicional e comissão persistida por item. Ela é reduzida quando faltam webhook, taxa de gateway normalizada, histórico temporal da configuração ou comprovante externo de saque.
